...
def plot_daily_forecast(df, forecast_end_date, model_selection):
    ...
    elif model_selection == 'StatsForecast ETS':
...
def statsforecast_ets(df, periods):
    ...
    with st.spinner('Training StatsForecast ETS model...'):
        models = [ETS(season_length=7)]  # Weekly seasonality
        sf = StatsForecast(
            models=models,
            freq='D',
            n_jobs=-1  # Use all available cores
...
def main():
    ...
    model_selection = st.sidebar.selectbox(
        ...
        ["Prophet", "NeuralProphet", "StatsForecast AutoARIMA", "StatsForecast ETS"],
        index=0
    )
    ...
    if model_selection == "Prophet":
        ...
        st.sidebar.info("Exponential Smoothing models are good for data with trend and seasonality but no complex patterns.")
