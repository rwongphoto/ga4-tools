...
import warnings
import torch # <-- Add for PyTorch >= 2.6 fix
...
# Import the specific Config classes needed for PyTorch >= 2.6 compatibility fix
from neuralprophet.configure import (
    ConfigSeasonality, ConfigExperiment, ConfigAr, ConfigCovariates, ConfigEvents,
    ConfigFutureRegressors, ConfigLaggedRegressors, ConfigModel, ConfigNormalization,
    ConfigOptimize, ConfigPerformance, ConfigTrend, ConfigCountryHolidays
)
...
def run_neural_prophet(df_hist, periods):
    ...
    # --- !!! ADD THIS SECTION FOR PYTORCH >= 2.6 !!! ---
    try:
        # Allowlist classes known to be used by NeuralProphet's config/checkpoints
        # Start with the one from the error, add others if needed based on potential future errors
        torch.serialization.add_safe_globals([
            ConfigSeasonality, ConfigExperiment, ConfigAr, ConfigCovariates, ConfigEvents,
            ConfigFutureRegressors, ConfigLaggedRegressors, ConfigModel, ConfigNormalization,
            ConfigOptimize, ConfigPerformance, ConfigTrend, ConfigCountryHolidays
            # Add any other neuralprophet.configure.* classes if they cause similar errors
        ])
        print("NeuralProphet safe globals added for PyTorch >= 2.6 compatibility.") # Optional: for confirmation
    except NameError:
        # Handle cases where torch might not be installed, though it's a dependency
        print("Torch not found, skipping safe_globals addition.")
    except Exception as e:
        st.warning(f"Could not add NeuralProphet safe globals (potential issue with PyTorch >= 2.6): {e}")
    ...
    model = NP(
        ...
        # Consider adding seasonality based on data patterns:
        # yearly_seasonality=True,
        # weekly_seasonality=True,
        # daily_seasonality=False # Usually False for daily data unless sub-daily patterns exist
    )

    # Fit the model - provide feedback as it can take time
    st.write("Fitting NeuralProphet model (this might take a few minutes)...")
    metrics = model.fit(df_np, freq='D')
    st.write("NeuralProphet fitting complete.")

    # Create future dataframe
    future = model.make_future_dataframe(df_np, periods=periods)

    # Predict - provide feedback
    st.write("Generating NeuralProphet forecast...")
    forecast = model.predict(future)
    st.write("NeuralProphet forecast generation complete.")
    ...
    # Return only the necessary columns in a standardized format
...
def run_statsforecast(df_hist, periods):
    ...
    # Ensure the input dataframe for forecasting is correctly formatted
    st.write("Fitting Statsforecast model and generating forecast...")
    forecast = sf.forecast(df=df_sf[['unique_id', 'ds', 'y']], h=periods, level=[95])
    st.write("Statsforecast processing complete.")

    # Standardize column names (adjust 'AutoARIMA' if using different/multiple models)
    model_name = model_list[0].__class__.__name__ # Dynamically get model name
...
def plot_forecast(df_actual, forecast, model_name, forecast_start_date):
    ...
    if 'yhat_lower' in forecast.columns and 'yhat_upper' in forecast.columns:
        ...
        # Ensure CI columns are numeric, handle potential NaNs
        future_forecast['yhat_lower'] = pd.to_numeric(future_forecast['yhat_lower'], errors='coerce')
        future_forecast['yhat_upper'] = pd.to_numeric(future_forecast['yhat_upper'], errors='coerce')
        future_forecast.dropna(subset=['yhat_lower', 'yhat_upper'], inplace=True) # Drop rows where CI is NaN
...
def display_dashboard(forecast, df_actual, last_date, forecast_end_date):
    ...
    if forecast_future.empty:
        ...
        forecast_future_display = forecast_future[display_cols].copy()
        forecast_future_display['ds'] = forecast_future_display['ds'].dt.strftime('%Y-%m-%d')

        # Round numeric columns
        numeric_cols = forecast_future_display.select_dtypes(include='number').columns
        forecast_future_display[numeric_cols] = forecast_future_display[numeric_cols].round(0).astype(int) # Round to whole numbers

        st.dataframe(forecast_future_display.set_index('ds')) # Set date as index for better display
    ...
    if not current_period_forecast_data.empty:
        ...
        if not prev_period_actual_data.empty:
            ...
            if prev_sum_actual != 0:
                ...
                yoy_string = f"{change_sign}{yoy_change:.2f}%"
            else:
                yoy_change = float('inf') # Avoid division by zero
                yoy_string = "N/A (Previous period sum is zero)"
            ...
            st.metric(label="Year-over-Year Change", value=f"{yoy_string}") # Use metric for better viz
        else:
            st.write(f"Total **Forecasted** Sessions ({start_forecast_period.date()} to {end_forecast_period.date()}): {current_sum_forecast:,.0f}")
...
def main():
    ...
    if df_raw is not None and last_date is not None:
        ...
        try:
            ...
            forecast_df = None
            with st.spinner(f"Running {selected_model}... this may take a while."):
                if selected_model == 'Prophet':
                    ...
                    # Important Note: Statsforecast `forecast` method usually returns only future dates.
                    # The plotting function `plot_forecast` is designed to handle this by taking
                    # the original `df_raw` for actuals and the potentially future-only `forecast_df`.
            ...
            if forecast_df is not None:
                ...
                plot_forecast(df_raw, forecast_df.copy(), selected_model, forecast_start_date) # Pass copy to plot
                display_dashboard(forecast_df.copy(), df_raw.copy(), last_date, forecast_end_date) # Pass copy to dashboard

                # --- Download Button ---
                # Offer download of the generated forecast data
                # Ensure 'ds' is consistently formatted before saving
                download_df = forecast_df.copy()
                download_df['ds'] = pd.to_datetime(download_df['ds']).dt.strftime('%Y-%m-%d')
                csv_data = download_df.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label=f"Download {selected_model} Forecast CSV",
                    data=csv_data,
                    file_name=f'{selected_model.lower().replace(" ", "_").replace("(", "").replace(")", "")}_forecast.csv',
                    mime='text/csv'
                )
            else:
                 st.error(f"Forecast generation failed for {selected_model}.")
